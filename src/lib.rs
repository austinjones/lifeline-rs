//! Lifeline is a dependency injection for asynchronous message-based applications.
//! Lifeline provides:
//! - The [Bus](./trait.Bus.html), which constructs & distributes channel Senders/Receivers, and Resources.
//! - The [Carrier](./trait.CarryFrom.html), which drives messages between Busses. Carriers are critical when building large applications.
//! - The [Service](./trait.Service.html), which takes channels from the bus, and drives messages along the channels.
